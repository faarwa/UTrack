...
_.extend(ActivityStoreModel.prototype, {
    ...
    addActivityDataPoint: function(activityDataPoint) {
        ...
        _.each(this.listeners, function(listener) {
            listener(activityDataPoint, ACTIVITY_DATA_ADDED_EVENT);
        })
    },
    ...
    removeActivityDataPoint: function(activityDataPoint) {
        ...
        if (index != -1) {
            delete this.dataPoints[index];

            _.each(this.listeners, function(listener) {
                listener(activityDataPoint, ACTIVITY_DATA_REMOVED_EVENT);
            })
        }
    },
    ...
    getHealthDictsForActivity: function(activityType) {
        var healthArray = [];
        _.each(this.dataPoints, function(dataPoint) {
            if (dataPoint.activityType == activityType) {
                healthArray.push(dataPoint.activityDataDict);
            }
        })
...
_.extend(GraphModel.prototype, {
    ...
    selectGraph: function(graphName) {
        ...
        _.each(this.listeners, function(listener) {
            listener(graphName, GRAPH_SELECTED_EVENT);
        })
    }
